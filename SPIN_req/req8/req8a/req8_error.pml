...
RequestToCreateNewCategoryScreen,NotificationScreen,ChangeusernameParm,ChangePasswordParm,ChangeAirplaneMode,ChangeWifi,changebluParam,ChangeWif};
...
ltl r7 {[]((wifiParam==1)->(!X(((state!=ChangeWifi))U(wifiParam==0))))}
...
:: (state== ChangeWifi) ->
	if	
	::(actions[0]==1 && actions[1]==1)->atomic{ state=changebluParam;bluParam=1;actions[1]=0;actions[2]=1}
...
:: (state== changebluParam) ->
	if	
	::(actions[0]==1 && actions[1]==1 && actions[2]==1)->atomic{actions[0]=0;actions[2]=0;state=LoginScreen}
	fi
