...
ltl  req1 {[](((state==Setting)->((!([]<>((state==changeBluetoothON)&&(state==changeWifiON)&&(state==changeBluetoothOFF)&&(state==changeAirplane_modeOFF)&&(state==changeAirplane_modeON)&&(state==changeWifiOFF))))-><>((state !=Setting)&&(state!=changeBluetoothOFF) &&  (state!=changeWifiOFF) && (state!=changeAirplane_modeON)))))}
...
active proctype vm(){
 do
	...
	::state==changeBluetoothOFF->
	  if
		 ::atomic{action[1]=0;state=changeAirplane_modeON};
	...
	::(state==changeAirplane_modeON)->
	  if
		 ::(action[2]==1 && action[0]==0)->atomic{action[2]=0;state=Setting};
